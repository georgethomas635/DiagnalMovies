package in.app.diagnalsample.apps;

public class Constants {

    public static final String PAGE = "page";
    public static final String CONTENT_ITEMS = "content-items";
    public static final String CONTENT = "content";
    public static final String MOVIE_NAME = "name";
    public static final String POSTER_IMAGE = "poster-image";
    public static final String FILE_NAME = "data/CONTENTLISTINGPAGE-PAGE";
    public static final String JSON_EXTENSION = ".json";
public static final String BUNDLE_MOVIES_KEY = "Movie List";
public static final String BUNDLE_SEARCH_MOVIES = "Serched Movie List";
    public static final int COLOUM_SIZE = 3;
    public static final int COLOUM_SIZE_LANDSCAPE = 7;
    public static final int ZERO = 0;
    public static final int RECYCLERVIEW_MARGIN = 30;
    public static final long TIME_DELAY = 2000;
    public static final int TOTAL_ITEM_COUNT = 20;
public static final String EMPTY = "";
}
